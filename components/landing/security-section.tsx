...
const featureMeta = [
  { icon: Shield, image: "/images/isolated.jpg" },
  { icon: Lock, image: "/images/encrypted.jpg" },
  { icon: Eye, image: "/images/audit.jpg" },
  { icon: FileCheck, image: "/images/permissions.jpg" },
];
...
export function SecuritySection() {
  const { t } = useLanguage();
  const securityFeatures = featureMeta.map((m, i) => ({ ...m, ...t.security.features[i] }));
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="mb-20">
          ...
          <h2 className={`text-6xl md:text-7xl lg:text-[128px] font-display tracking-tight leading-[0.9] mb-12 transition-all duration-1000 ${
            ...
            {t.security.title1}
            <br />
            <span className="text-muted-foreground">{t.security.title2}</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          ...
          <div className={`lg:col-span-7 relative p-8 lg:p-12 border border-foreground/10 min-h-[400px] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <span className="font-mono text-sm text-muted-foreground">{t.security.active}</span>
              <div className="mt-8">
                <span className="text-7xl lg:text-8xl font-display">{t.security.bigNumber}</span>
                <span className="block text-muted-foreground mt-2">{t.security.bigLabel}</span>
              </div>
            </div>
