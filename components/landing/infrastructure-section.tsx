...
export function InfrastructureSection() {
  const { t } = useLanguage();
  const capabilities = t.autonomy.capabilities;
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveRegion((prev) => (prev + 1) % capabilities.length);
    }, 3000);
    return () => clearInterval(interval);
  }, [capabilities.length]);

  return (
    <section id="autonomy" ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              <h2 className={`text-6xl md:text-7xl lg:text-[128px] font-display tracking-tight leading-[0.9] transition-all duration-1000 ${
                ...
                {t.autonomy.title1}
                <br />
                <span className="text-muted-foreground">{t.autonomy.title2}</span>
              </h2>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-8xl lg:text-[10rem] font-display leading-none">{t.autonomy.bigNumber}</span>
                <span className="text-2xl text-muted-foreground">{t.autonomy.bigUnit}</span>
              </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="text-5xl lg:text-6xl font-display">{t.autonomy.card1Value}</span>
              <span className="block text-sm text-muted-foreground mt-2">{t.autonomy.card1Label}</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">{t.autonomy.card2Value}</span>
              <span className="block text-sm text-muted-foreground mt-2">{t.autonomy.card2Label}</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {capabilities.map((cap, index) => (
            ...
            >
              ...
              <span className="font-medium block mb-1">{cap.name}</span>
              <span className="text-sm text-muted-foreground">{cap.detail}</span>
            </div>
          ))}
        </div>
      </div>
